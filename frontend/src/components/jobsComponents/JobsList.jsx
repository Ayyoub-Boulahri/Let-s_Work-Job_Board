...
function JobsList() {
    ...
    const handlePageChange = (newPage) => {
        ...
        const arrayJobs = jobs.slice(
            (newPage - 1) * jobsPerPage,
            newPage * jobsPerPage
        );
        setVisibleJobs(arrayJobs)
        window.scrollTo({ top: 400 });
    };

    const defaultJobs = jobs.slice(
        (currentPage - 1) * jobsPerPage,
        currentPage * jobsPerPage
    );

    const [visibleJobs, setVisibleJobs] = useState(defaultJobs);
